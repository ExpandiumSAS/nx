#ifndef __NX_WATCHER_H__
#define __NX_WATCHER_H__
...
class watcher : public watcher_base
{
public:
    using watcher_type = Watcher;
    using this_type = watcher<Derived, Watcher>;
    ...
    watcher(this_type&& other)
    { *this = std::move(other); }

    watcher(const this_type& other) = delete;
    ...
    void start() noexcept
    { async() << [&](evloop el) { start_func_(el, wptr()); }; }

    void stop() noexcept
    { async() << [&](evloop el) { stop_func_(el, wptr()); }; }
    ...
    { return ev_is_active(wptr()); }

    bool is_pending() const noexcept
    { return ev_is_pending(wptr()); }
